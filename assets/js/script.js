...
function scaleOfHash(num){
    let scaleHash = ""
    for (let i = 0; i < num; i++){
        scaleHash += "#"
        console.log(scaleHash)
    }
}

console.log(scaleOfHash(3))
...
function reversText(str){
    ...
    let returnString = arrStr.toString().replaceAll(",", "")
    return returnString
}

console.log(reversText("Ciao"))
...
function splitNumber(arr, num){
    let newArray = []
    for(let i = 0; i < arr.length; i += num){
        let newSubArray = arr.slice(i, i + num)
        newArray.push(newSubArray)
    }
    return newArray
}

let groupNum = [1, 2, 3, 4,]
console.log(splitNumber(groupNum, 2))
...
function piramideOfHash(num){
    let piramideOfHash = ""
    for (let i = 0; i < num; i++){
        piramideOfHash += "#"
        console.log(piramideOfHash)
    }
}
...
//Devo ancora migliorare molto sotto js... quindi preferisco consegnarlo senza gli ultimi due esercizi